...
from main.network.models import encoder_latent, decoder
...
class OccupancyNetwork(nn.Module):
    ...
    def __init__(self, decoder, encoder=None, encoder_latent=None, p0_z=None, voxel_decoder=None,
                 device=None):
        super().__init__()
        if p0_z is None:
        ...
        if voxel_decoder is not None:
            self.voxel_decoder = voxel_decoder.to(device)
        else:
    ...
    def decode(self, p, z, c, **kwargs):
        ...
        if self.voxel_decoder is not None:
            voxel = self.voxel_decoder(z, c)
            return p_r, voxel
        return p_r
